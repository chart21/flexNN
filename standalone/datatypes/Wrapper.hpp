...
class Wrapper
{
  ...
  public:
    ...
    Wrapper(T s, int dummy) { this->s1 = s; }

    Wrapper() { this->s1 = 0; }

    T get_s1() { return this->s1; }

    Wrapper operator+(const Wrapper s) const { return Wrapper(this->s1 + s.s1, 0); }

    Wrapper operator-(const Wrapper s) const { return Wrapper(this->s1 - s.s1, 0); }

    Wrapper operator*(const Wrapper s) const { return Wrapper(this->s1 * s.s1, 0); }

    Wrapper operator*(const int s) const { return Wrapper(this->s1 * s, 0); }

    Wrapper operator/(const int s) const { return Wrapper(this->s1 / s, 0); }

    void mask_and_send_dot() {}

    void complete_mult() { this->s1 = truncate(this->s1); }

    void operator/=(const int s) { this->s1 /= s; }

    void operator+=(const Wrapper s) { this->s1 += s.s1; }

    void operator-=(const Wrapper s) { this->s1 -= s.s1; }

    void operator*=(const Wrapper s) { *this = *this * s; }

    bool operator==(const W& other) const { return false; }
    ...
    W operator/(const W& other) const { return W(this->s1 / other.s1, 0); }

    W exp() const { return W(std::exp(this->s1), 0); }

    W log() const { return W(std::log(this->s1), 0); }
    ...
    static void communicate() { /* std::cout << "communicate" << std::endl; */ }
