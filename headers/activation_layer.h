...
class Tanh : public Activation<T>
{
  public:
    ...
    void forward(const MatX<T>& prev_out, bool is_training) override
    {
        ...
        std::transform(
            prev_out.data(), prev_out.data() + this->out_block_size, this->output.data(), [](const float& e) {
                return 2 / (1 + std::exp(-2.f * e)) - 1;
            });
    }

    void backward(const MatX<T>& prev_out, MatX<T>& prev_delta) override
    {
        std::transform(prev_out.data(),
...
{
    T out = std::accumulate(p, p + size, T(0), [&](const T& sum, const T& elem) { return sum + (elem - max).exp(); });
    return out;
}
  ...
  public:
    ...
    void forward(const MatX<T>& prev_out, bool is_training) override
    {
        ...
        for (int n = 0; n < this->batch; n++)
        {
            ...
            std::transform(begin, begin + this->height, this->output.data() + offset, [&](const T& e) {
