...
class DataLoader
{
  ...
  public:
    DataLoader();
    DataLoader(MatX<T>& X, VecXi& Y, int batch, int channels, int height, int width, bool shuffle);
    void load(MatX<T>& X, VecXi& Y, int batch, int channels, int height, int width, bool shuffle);
    int size() const;
...
template <typename T>
DataLoader<T>::DataLoader() : n_batch(0), batch(0), ch(0), h(0), w(0), chhw(0)
{
}

template <typename T>
DataLoader<T>::DataLoader(MatX<T>& X, VecXi& Y, int batch, int channels, int height, int width, bool shuffle)
    : X(std::move(X)),
      Y(std::move(Y)),
      n_batch(0),
      batch(batch),
...
template <typename T>
void DataLoader<T>::load(MatX<T>& X, VecXi& Y, int batch, int channels, int height, int width, bool shuffle)
{
    this->X = std::move(X);
    this->Y = std::move(Y);
    this->batch = batch;
...
template <typename T>
int DataLoader<T>::size() const
{
    return n_batch;
}

template <typename T>
vector<int> DataLoader<T>::input_shape() const
{
    return {batch, ch, h, w};
}
