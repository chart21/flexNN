...
void SimpleNN<T>::classify(const MatX<T>& output, VecXi& classified)
{
    ...
    for (int i = 0; i < output.rows(); i++)
    {
        for (int j = 0; j < output.cols(); j++)
        {
            ...
            {
                output_float(i * (BASE_DIV) + k, j) =
                    FloatFixedConverter<FLOATTYPE, INT_TYPE, UINT_TYPE, FRACTIONAL>::ufixed_to_float(tmp[k]);
            }
        }
    }
#else
    ...
    for (int i = 0; i < output.rows(); i++)
    {
        for (int j = 0; j < output.cols(); j++)
        {
            ...
            output_float(i, j) = FloatFixedConverter<FLOATTYPE, INT_TYPE, UINT_TYPE, FRACTIONAL>::ufixed_to_float(
...
void SimpleNN<T>::prepare_read_params(fstream& fs)
{
    for (Layer<T>* l : net)
    {
        ...
        if (l->type == LayerType::LINEAR)
        {
            ...
            {
#if PUBLIC_WEIGHTS == 0
                lc->W(i / lc->W.cols(), i % lc->W.cols())
                    .template prepare_receive_and_replicate<id>(
                        FloatFixedConverter<FLOATTYPE, INT_TYPE, UINT_TYPE, FRACTIONAL>::float_to_ufixed(
                            tempMatrix1[i]));
#else
                lc->W(i / lc->W.cols(), i % lc->W.cols()) =
                    FloatFixedConverter<FLOATTYPE, INT_TYPE, UINT_TYPE, FRACTIONAL>::float_to_ufixed(tempMatrix1[i]);
#endif
            }
            for (int i = 0; i < s2; i++)
            {
#if PUBLIC_WEIGHTS == 0
                lc->b[i].template prepare_receive_and_replicate<id>(
                    FloatFixedConverter<FLOATTYPE, INT_TYPE, UINT_TYPE, FRACTIONAL>::float_to_ufixed(tempMatrix2[i]));
#else
                lc->b[i] =
                    FloatFixedConverter<FLOATTYPE, INT_TYPE, UINT_TYPE, FRACTIONAL>::float_to_ufixed(tempMatrix2[i]);
#endif
        ...
        else if (l->type == LayerType::CONV2D)
        {
            ...
            {
#if PUBLIC_WEIGHTS == 0
                lc->kernel(i / lc->kernel.cols(), i % lc->kernel.cols())
                    .template prepare_receive_and_replicate<id>(
                        FloatFixedConverter<FLOATTYPE, INT_TYPE, UINT_TYPE, FRACTIONAL>::float_to_ufixed(
                            tempMatrix1[i]));
            ...
            {
#if PUBLIC_WEIGHTS == 0
                lc->bias[i].template prepare_receive_and_replicate<id>(
                    FloatFixedConverter<FLOATTYPE, INT_TYPE, UINT_TYPE, FRACTIONAL>::float_to_ufixed(tempMatrix2[i]));
#else
                lc->bias[i] =
                    FloatFixedConverter<FLOATTYPE, INT_TYPE, UINT_TYPE, FRACTIONAL>::float_to_ufixed(tempMatrix2[i]);
        ...
        else if (l->type == LayerType::BATCHNORM1D)
        {
            ...
            {
#if PUBLIC_WEIGHTS == 0
                lc->move_mu[i].template prepare_receive_and_replicate<id>(
                    FloatFixedConverter<FLOATTYPE, INT_TYPE, UINT_TYPE, FRACTIONAL>::float_to_ufixed(tempMatrix1[i]));
#else
                lc->move_mu[i] =
                    FloatFixedConverter<FLOATTYPE, INT_TYPE, UINT_TYPE, FRACTIONAL>::float_to_ufixed(tempMatrix1[i]);
            ...
            {
                double new_var =
                    static_cast<double>(tempMatrix3[i]) /
                    std::sqrt(static_cast<double>(tempMatrix2[i]) +
                              0.00001);  // Optimiation, fuse the division with the square root and gamma multiplication
                 ...
                 */
#if PUBLIC_WEIGHTS == 0
                lc->move_var[i].template prepare_receive_and_replicate<id>(
                    FloatFixedConverter<FLOATTYPE, INT_TYPE, UINT_TYPE, FRACTIONAL>::float_to_ufixed(new_var));
#else
                lc->move_var[i] =
                    FloatFixedConverter<FLOATTYPE, INT_TYPE, UINT_TYPE, FRACTIONAL>::float_to_ufixed(new_var);
#endif
            ...
            /* { */
            /* #if PUBLIC_WEIGHTS == 0 */
            /*     lc->gamma[i].template prepare_receive_and_replicate<id>(FloatFixedConverter<float, INT_TYPE,
             * UINT_TYPE, FRACTIONAL>::float_to_ufixed(tempMatrix3[i])); */
            /* #else */
            /*     lc->gamma[i] = FloatFixedConverter<float, INT_TYPE, UINT_TYPE,
             * FRACTIONAL>::float_to_ufixed(tempMatrix3[i]); */
            /* #endif */
            /* } */
            for (int i = 0; i < s4; i++)
            {
#if PUBLIC_WEIGHTS == 0
                lc->beta[i].template prepare_receive_and_replicate<id>(
                    FloatFixedConverter<FLOATTYPE, INT_TYPE, UINT_TYPE, FRACTIONAL>::float_to_ufixed(tempMatrix4[i]));
#else
                lc->beta[i] =
                    FloatFixedConverter<FLOATTYPE, INT_TYPE, UINT_TYPE, FRACTIONAL>::float_to_ufixed(tempMatrix4[i]);
#endif
            ...
            for (int i = 0; i < s2; i++)
            {
                double new_var =
                    ...
                    std::sqrt(static_cast<double>(tempMatrix2[i]) +
                              0.00001);  // Optimiation, fuse the division with the square root and gamma multiplication
                                         /* float var = static_cast<float>(new_var); */
                /* float var = tempMatrix3[i] / std::sqrt(tempMatrix2[i] + 0.00001f); // Optimiation, fuse the division
                 * with the square root and gamma multiplication */
                /* if(MODELOWNER == PSELF && current_phase == PHASE_LIVE) */
                /*      std::cout << "gamma" << tempMatrix3[i] << "var" << tempMatrix2[i] << "new var" << var << "\n";
                 */
...
void SimpleNN<T>::evaluate(const DataLoader<F>& data_loader)
{
    ...
    for (int n = 0; n < n_batch; n++)
    {
        ...
        for (int j = 0; j < test_X.cols(); j++)
        {
            for (int i = 0; i < test_X.rows(); i += BASE_DIV * ch)
            {
                ...
                for (int c = 0; c < ch; c++)
                    for (int k = 0; k < BASE_DIV; ++k)
                    {
                        tmp[c][k] = FloatFixedConverter<FLOATTYPE, INT_TYPE, UINT_TYPE, FRACTIONAL>::float_to_ufixed(
                            test_X(i + k * ch + c, j));
                    }
                for (int c = 0; c < ch; c++)
                {
        ...
        {
            cout << "P" << PARTY << ", PID" << process_offset << ": "
                 << "[Batch: " << setw(3) << (n + 1) + n_batch * process_offset << "/" << n_batch * (process_offset + 1)
                 << "]";
            if (n + 1 < n_batch)
            {
                cout << "\r" << flush;
